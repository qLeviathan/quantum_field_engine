...
# --- Expand QFNN field by generating new tokens ---
class QFNNFieldExpander:
    def __init__(self, model_name="gpt2", max_tokens=5000):
        ...
        self.token_list = list(self.token_bank.keys())
        self.phase_vectors = torch.stack([self.token_bank[tok] for tok in self.token_list]).to(DEVICE)

    def expand_field(self, prompt, num_new_tokens=10, steps_per_evolve=500):
...
if __name__ == "__main__":
    expander = QFNNFieldExpander()
    prompt = "the cosmos dances with light and"
